...
load("net_Q_support.RData") #(outliers and missing data removed)
...
edges <- as.data.frame(as.edgelist(net_Q_support))

edges$kin_relatedness <- get.edge.attribute(net_Q_support, "kinship_coeff")
edges$distance <- get.edge.attribute(net_Q_support, "Dyadic_distance")
...
summary(net_Q_support)
list.vertex.attributes(net_Q_support)
list.edge.attributes(net_Q_support)

#trim incomplete wealth data
complete_wealth <- !is.na(net_Q_support %v% "MaterialWealth")

#create new network with complete cases
net_Q_clean <- net_Q_support
net_Q_clean <- delete.vertices(net_Q_clean, which(!complete_wealth))

#trim dyadic distances 
summary(net_Q_clean %e% "Dyadic_distance")
sum(is.na(net_Q_clean %e% "Dyadic_distance"))
complete_dist <- !is.na(net_Q_clean %e% "Dyadic_distance")

#create new network without NA distance edges
net_Q_support <- delete.edges(net_Q_clean, which(!complete_dist))

#look at actual values
head(net_Q_support %e% "Dyadic_distance", 100) #89 observations for distance (because there are 88 dyads)

#*pre-model diagnostic checks* 
#extract variables of interest relevant to H1
material_wealth <- get.vertex.attribute(net_Q_support, "MaterialWealth")
age <- get.vertex.attribute(net_Q_support, "Age")
gender <- get.vertex.attribute(net_Q_support, "Gender")
dyadic_distance <- get.edge.attribute(net_Q_support, "Dyadic_distance")
...
png("QSUPPORT_boxplots_outlier_detection.png", width = 10, height = 8, units = "in", res = 300)
par(mfrow = c(2, 2))
boxplot(kin_density, main = "Kin Density", ylab = "Density")
boxplot(material_wealth, main = "Material Wealth", ylab = "Wealth")
boxplot(age, main = "Age", ylab = "Years")
...
dev.off()
cat("Boxplots exported to: QSUPPORTboxplots_outlier_detection.png\n")
...
material_wealth <- net_Q_support %v% "MaterialWealth"
age <- net_Q_support %v% "Age"
dyadicdistance <- net_Q_support %e% "Dyadic_distance"

get.edge.attribute(net_Q_support, "Dyadic_distance")

#remove distance outliers
net_Q_support <- delete.edges(net_Q_support , c(69, 99, 122, 142))

get.edge.attribute(net_Q_support , "Dyadic_distance") #outliers have been removed! so 52 dyads remaining
...
class(net_Q_support)
network.size(net_Q_support) #54 egos in network (now outliers and missing data from key variables are removed)
...
node_data <- data.frame(
  Kin_density_Qsupport = net_Q_support_clean %v% "Kin_density_Qsupport",
  MaterialWealth = net_Q_support_clean %v% "MaterialWealth",
  Age = net_Q_support_clean %v% "Age",
  Gender = net_Q_support_clean %v% "Gender"
)
...
kin_density <- net_Q_support_clean %v% "Kin_density_Qsupport"
material_wealth <- net_Q_support_clean %v% "MaterialWealth"
age <- net_Q_support_clean %v% "Age"
gender <- net_Q_support_clean %v% "Gender"
...
node_data <- data.frame(
  Kin_density_Qsupport = as.numeric(kin_density),
...
save(net_Q_support, file = "net_Q_support_clean.RData")
load("net_Q_support_clean.RData")

#[2] matrix formation####

#inspect attributes 
class(net_Q_support)
as.edgelist(net_Q_support) #dyadic attributes 
edges_support <- as.data.frame.network(net_Q_support, matrix.type = "edgelist") #
summary(net_Q_support) #summary of 143 edges 

list.vertex.attributes(net_Q_support) #list all dyadic attributes 
list.edge.attributes(net_Q_support) #list all edge attributes 

net_Q_support
plot(net_Q_support)
summary(net_Q_support)
length(unique(edges_support$.tail)) #52 egos 
length(unique(edges_support$.head)) #41 alters 
network.size(net_Q_support) #55
network.edgecount(net_Q_support) #143 edges in total
network.density(net_Q_support) #network density = 0.04727273
summary(net_Q_support ~ idegree(0:10) + odegree(0:10)) #indegree and outdegree descriptives
deg_summary <- summary(net_Q_support ~ idegree(0:10) + odegree(0:10))
deg_summary

library(sna)
recip_value <- reciprocity(net_Q_support)
print(recip_value)

Q_degree_plot <- barplot(deg_summary,
...
#--> material wealth 
summary(get.vertex.attribute(net_Q_support, "MaterialWealth"))
sd(get.vertex.attribute(net_Q_support, "MaterialWealth"), na.rm = TRUE)

#--> age 
summary(get.vertex.attribute(net_Q_support, "Age"))
sd(get.vertex.attribute(net_Q_support, "Age"), na.rm = TRUE)

#--> dyadic distance
dyadic_distance <- get.edge.attribute(net_Q_support, "Dyadic_distance")
summary(dyadic_distance)
IQR(get.edge.attribute(net_Q_support, "Dyadic_distance"))

#--> kin coef
kin <- get.edge.attribute(net_Q_support, "kinship_coeff")
summary(kin)
sd(get.edge.attribute(net_Q_support, "kinship_coeff"))
...
network_verts <- network.vertex.names(net_Q_support)

pairwise_distances_Q_support <- pairwise_distances_full %>%
  filter(Ego %in% network_verts & Alter %in% network_verts)

#checks
unique_egos <- unique(c(pairwise_distances_Q_support$Ego, pairwise_distances_Q_support$Alter))
...
distance_Q_support_matrix <- pairwise_distances_Q_support%>%
...
#match vertex order
vertex_order <- network.vertex.names(net_Q_support)
distance_Q_support_matrix <- distance_Q_support_matrix[vertex_order, vertex_order]
...
kinship_Q_support_matrix <- pedigree_dyads_Q_support %>%
...
kinship_Q_support_matrix <- kinship_Q_support_matrix[vertex_order, vertex_order]

#replace any NAs (optional; replace with 0 or mean)
kinship_Q_support_matrix[is.na(kinship_Q_support_matrix)] <- 0
kinship_Q_support_matrix <- as.matrix(kinship_Q_support_matrix)

#finding missing vertex

n <- network.size(net_Q_support)
vertex_names <- network.vertex.names(net_Q_support)
print(n)              # Should be 54
print(vertex_names)   

setdiff(vertex_names, rownames(kinship_Q_support_matrix))
setdiff(rownames(kinship_Q_support_matrix), vertex_names)

vertex_names <- network.vertex.names(net_Q_support)

#reorder rows and columns to network vertex order
kinship_Q_support_matrix <- kinship_Q_support_matrix[vertex_names, vertex_names]

#rescale 
kinship_Q_support_matrix_rescaled <- kinship_Q_support_matrix / 0.1
...
model_Q_support_1 <- ergm(net_Q_support ~ edges, 
                     control = control.ergm(MCMLE.maxit = 100, MCMC.samplesize = 200,MCMC.burnin = 200, MCMC.interval = 100,seed = 123))

model_Q_support_2 <- ergm(net_Q_support ~ edges 
                     + mutual + gwesp(decay = 0.5, fixed = TRUE) + gwdsp(decay = 0.5),
                     control = control.ergm(MCMLE.maxit = 100, MCMC.samplesize = 200,MCMC.burnin = 200, MCMC.interval = 100,seed = 123))

model_QNBT_3 <- ergm(net_Q_support ~ edges 
                     + mutual + gwesp(decay = 0.5, fixed = TRUE) + gwdsp(decay = 0.5)
                     + nodecov("Age") + nodematch("Gender") + nodefactor ("Gender"), 
                     control = control.ergm(MCMLE.maxit = 100, MCMC.samplesize = 200,MCMC.burnin = 200, MCMC.interval = 100,seed = 123))

model_QNBT_4 <- ergm(net_Q_support ~ edges 
                     + mutual + gwesp(decay = 0.5, fixed = TRUE) + gwdsp(decay = 0.5)
                     + nodecov("Age") + nodematch("Gender") + nodefactor ("Gender") 
                     + nodecov("MaterialWealth") + nodeocov("MaterialWealth") + absdiff("MaterialWealth"),
                     control = control.ergm(MCMLE.maxit = 100, MCMC.samplesize = 200,MCMC.burnin = 200, MCMC.interval = 100,seed = 123))

model_Q_support_5 <- ergm(net_Q_support ~ edges 
                     + mutual + gwdsp(decay = 0.5, fixed = TRUE) + gwesp(decay = 0.5, fixed = TRUE) 
                     + nodecov("Age") + nodematch("Gender") + nodefactor("Gender") 
                     + edgecov(kinship_Q_support_matrix_rescaled) +  edgecov(distance_Q_support_matrix),
                     control = control.ergm(MCMLE.maxit = 100, MCMC.samplesize = 200,MCMC.burnin = 200, MCMC.interval = 100,seed = 123))

save(model_QNBT_1, model_QNBT_2, model_QNBT_3, model_QNBT_4, model_QNBT_5, file = "models_QNBT_all.RData")
load("models_Q_support_all.RData")

summary(model_Q_support_1) # baseline (edges only)
summary(model_Q_support_2) # structural (mutual, GWESP, GDESP)
summary(model_Q_support_3) # control (age and gender)
summary(model_Q_support_4) # covariates (wealth)
summary(model_Q_support_5) # key predictors


AIC(model_Q_support_1, model_Q_support_2, model_Q_support_3, model_Q_support_4, model_Q_support_5)
logLik(model_Q_support_1)
logLik(model_Q_support_2)
logLik(model_Q_support_3)
logLik(model_Q_support_4)
logLik(model_Q_support_5)


length(coef(model_Q_support_1)) 
length(coef(model_Q_support_2)) 
length(coef(model_Q_support_3)) 
length(coef(model_Q_support_4)) 
length(coef(model_Q_support_5)) 
length(coef(model_Q_support_6)) 


#model_Qsupport_6 (best fitting model) report logistic regression results 
#regression coefficients 
a <-  -4.003 #edges
b1 <- 1.080e+00 #mutual
b2 <-  5.961e-01#kinship matrix
b3 <- 1.033e-05 #ego material wealth
b4 <-   -1.357e-06 #alter material wealth
b5 <-  1.020e-05 #wealth homophily
b6 <-  9.849e-03 #age 
b7 <-   2.349e-01 #gender homophily
b8 <-  2.123e-01 #gender (m)
b9 <-  -4.057e-05 #distance matrix 
b10 <-  1.304e+00 #edge-wise
b11 <-  -2.665e-01 #dyad-wise
...
exp(b3)  #ego wealth 
exp(b4)  #alter wealth
exp(b5)  #wealth homophily
exp(b6)  #age
exp(b7)  #gender homophily
exp(b8)  #gender (m)
exp(b9)  #distance
exp(b10) #e-w
exp(b11) #d-w
...
exp(confint(model_Q_support_5)) #CI intervals
exp(a)/(1+exp(a)) #predicted proportion for the baseline group = 0.0158993

#table of results 
summary_model <- summary(model_Q_support_5)
...
#GOOF
gof_step1 <- gof(model_Q_support_5)
print(gof_step1)
plot(gof_step1)
par(mfrow = c(2, 3))
plot(gof_step1)
...
mcmc.diagnostics(model_Qsupport_6)
